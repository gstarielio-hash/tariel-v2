...
def test_e2e_admin_provisiona_admin_cliente_e_portal_unificado_funciona(
    ...
    try:
        ...
        expect(page_cliente.locator("#admin-section-tab-overview")).to_have_attribute("aria-selected", "true")
        expect(page_cliente.locator("#hero-command-title")).to_contain_text("Primeiro acesso pendente", timeout=10000)
        acao_prioritaria = page_cliente.locator("#hero-command-action")
        expect(acao_prioritaria).to_contain_text("Revisar equipe", timeout=10000)
        acao_prioritaria.click()
