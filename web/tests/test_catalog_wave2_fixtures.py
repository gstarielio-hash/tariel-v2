...
@pytest.mark.parametrize("family_key", WAVE_2_FAMILIES)
def test_wave2_family_artifacts_exist_and_are_coherent(family_key: str) -> None:
...
@pytest.mark.parametrize("family_key", WAVE_2_FAMILIES)
def test_wave2_build_catalog_pdf_payload_preserva_fixture_canonico(family_key: str) -> None:
    output_example = _load_fixture(f"{family_key}.laudo_output_exemplo.json")
    template_seed = _load_fixture(f"{family_key}.template_master_seed.json")
    ...
    laudo = SimpleNamespace(
        ...
        catalog_variant_label="fixture_wave_2",
        status_revisao=StatusRevisao.APROVADO.value,
        setor_industrial="fixture",
        parecer_ia="Fixture canonico da onda 2",
