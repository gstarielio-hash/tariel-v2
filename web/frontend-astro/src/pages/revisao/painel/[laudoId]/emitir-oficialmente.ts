...
export const POST: APIRoute = async (context) => {
  ...
  const signatoryId = resolveReviewerMesaInt(formData.get("signatoryId")?.toString() ?? null);
  ...
  try {
    const result = await issueReviewerMesaOfficial(reviewerSession, {
      laudoId,
      signatoryId,
      expectedCurrentIssueId: resolveReviewerMesaInt(formData.get("expectedCurrentIssueId")?.toString() ?? null),
