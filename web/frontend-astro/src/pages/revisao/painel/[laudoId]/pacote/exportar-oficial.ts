...
import { fetchReviewerMesaOfficialZip } from "@/lib/server/reviewer-mesa";
...
export const GET: APIRoute = async (context) => {
  ...
  try {
    const upstream = await fetchReviewerMesaOfficialZip(reviewerSession, laudoId);
    return buildProxyResponse(upstream);
  } catch (error) {
    const detail = error instanceof Error && error.message.trim() ? error.message : "Falha ao exportar o pacote oficial.";
