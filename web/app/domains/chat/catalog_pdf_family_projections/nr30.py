...
_NR30_FAMILY_KEY = "nr30_inspecao_trabalho_aquaviario"
_NR30_DOCUMENT_NOTE = "Documento vinculado ao contexto de seguranca e saude no trabalho aquaviario."
_NR30_DEFAULT_METHOD = (
    "Inspecao de campo em trabalho aquaviario com checklist NR30, verificacao de acessos, "
    "condicoes de bordo, comunicacao operacional e controles de emergencia."
)


def _resolve_nr30_conclusion_status(
    *values: Any,
    review_status: Any,
...
def apply_nr30_projection(
    *,
    ...
    if family_key != _NR30_FAMILY_KEY:
        return
    ...
    internal_code = _pick_text_by_paths(
        existing_payload,
        payload,
        paths=["identificacao.codigo_interno", "codigo_interno", "numero_viagem", "embarcacao_id", "tag"],
    )
    ...
    main_reference_desc = _pick_first_text(
        ...
        "Referencia principal do cenario aquaviario validada com evidencia de campo." if main_reference_text else None,
        object_hint,
    )
    ...
    nr30_location_hint = _pick_first_text(
        _pick_text_by_paths(
            existing_payload,
            payload,
            paths=["identificacao.localizacao", "local_inspecao", "localizacao", "embarcacao", "terminal", "setor", "unidade"],
        ),
        location_hint,
    )
    ...
    relevant_parameters = _build_labeled_summary(
        ("Tipo embarcacao", _pick_text_by_paths(existing_payload, payload, paths=["tipo_embarcacao", "embarcacao_tipo"])),
        ("Atividade bordo", _pick_text_by_paths(existing_payload, payload, paths=["atividade_bordo", "atividade_principal"])),
        ("Acesso embarcacao", _pick_text_by_paths(existing_payload, payload, paths=["acesso_embarcacao", "acesso_operacional"])),
        ("Coletes e EPIs", _pick_text_by_paths(existing_payload, payload, paths=["coletes_epis", "epis", "epi"])),
        ("Comunicacao", _pick_text_by_paths(existing_payload, payload, paths=["comunicacao_operacional", "comunicacao"])),
        ("Estabilidade", _pick_text_by_paths(existing_payload, payload, paths=["estabilidade_operacao", "estabilidade"])),
        ("Abandono emergencia", _pick_text_by_paths(existing_payload, payload, paths=["abandono_emergencia", "rota_fuga"])),
        ("Conves e piso", _pick_text_by_paths(existing_payload, payload, paths=["condicoes_conves", "piso_operacional", "conves"])),
    )
    ...
    pgr = _pick_text_by_paths(existing_payload, payload, paths=["pgr_embarcacao", "pgr"])
    ...
    checklist = _pick_text_by_paths(existing_payload, payload, paths=["checklist_bordo", "checklist_operacional"])
    art_number = _pick_text_by_paths(existing_payload, payload, paths=["art_numero", "art"])
    document_base_text = _pick_first_text(pgr, procedure, emergency_plan, checklist)
    ...
    _set_path_if_blank(payload, "identificacao.localizacao", nr30_location_hint)
    ...
    _set_path_if_blank(payload, "escopo_servico.ativo_tipo", "trabalho_aquaviario")
    ...
    _set_block_fields_if_blank(
        ...
        description="Documento base principal considerado para a avaliacao NR30." if document_base_text else None,
    ...
    _set_path_if_blank(
        payload,
        "conclusao.status",
        _resolve_nr30_conclusion_status(
            explicit_conclusion,
