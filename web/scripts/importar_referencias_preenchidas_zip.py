...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Importa um pacote ZIP de referencias preenchidas promovendo o bundle "
            "oficial para a workspace canonica da familia."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Parametro legado preservado por compatibilidade. "
            "Nao altera a promocao documental da workspace."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def main() -> int:
    ...
    if args.dry_run:
        preflight["empresa_id"] = int(args.empresa_id)
    ...
    payload = promote_reference_package_to_workspace(
        zip_path=zip_path,
        workspace_root=workspace_root,
        pdf_path=pdf_path,
        validation_date=args.validation_date,
    )
    payload["empresa_id"] = int(args.empresa_id)
    payload["mode"] = "workspace_reference_package_promote"
