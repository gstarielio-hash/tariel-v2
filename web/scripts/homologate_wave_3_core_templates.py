...
OUTPUT_DOC_PATH = REPO_ROOT / "web" / "docs" / "onda_3_homologacao_profissional.md"
...
WAVE_3_ACTIVE_FAMILIES: tuple[str, ...] = WAVE_3_FAMILIES
...
def _build_professional_seed(family_key: str) -> dict[str, Any]:
    ...
    payload["observacoes"] = (
        f"{str(payload.get('observacoes') or '').strip()} | Homologado para a Onda 3 nacional com acabamento profissional e emissao demo controlada."
    ).strip()
    return payload
...
def _build_doc(rows: list[dict[str, Any]], demos: list[dict[str, Any]]) -> str:
    lines = [
        "# Onda 3: Homologacao Profissional",
        "",
        "Homologacao controlada da Onda 3 nacional com seeds profissionais, rollout v2 e demos emitidos no tenant piloto.",
        "",
    ...
    lines.extend(
        [
            ...
            "## Regra de promocao",
            "",
            "- todas as familias da Onda 3 foram promovidas para `ativo` na versao homologada;",
            "- cada familia possui seed profissional `v2` e demo emitida no tenant piloto;",
            "- a promocao foi aplicada sem tocar os ativos ja homologados fora da Onda 3.",
        ]
    )
    return "\n".join(lines) + "\n"
...
async def _emit_demo_document(
    *,
    ...
    laudo = Laudo(
        ...
        parecer_ia="Demo homologada da Onda 3 com pacote profissional.",
        catalog_family_key=family_key,
        catalog_family_label=str(schema.get("nome_exibicao") or family_key),
        catalog_variant_key="wave_3_homologation",
        catalog_variant_label="Homologação Onda 3",
        codigo_hash=uuid.uuid4().hex,
        primeira_mensagem=f"Demo Onda 3 {family_key}"[:80],
    )
    db.add(laudo)
    db.flush()
    ...
    payload = build_catalog_pdf_payload(
        ...
        diagnostico=str(exemplo.get("resumo_executivo") or f"Demo Onda 3 {family_key}"),
    ...
    manifest_path.write_text(
        json.dumps(
            {
                "source": "wave_3_homologation_script",
...
async def main() -> None:
    ...
    for family_key in WAVE_3_FAMILIES:
    ...
    with SessaoLocal() as db:
        ...
        for family_key in WAVE_3_FAMILIES:
            ...
            marcar_template_status(
                db,
                template=template,
                status_template="ativo" if family_key in WAVE_3_ACTIVE_FAMILIES else "em_teste",
            )
        ...
        commit_ou_rollback_integridade(
            db,
            logger_operacao=logging.getLogger("tariel.wave3.homologation"),
            mensagem_erro="Falha ao sincronizar templates homologados da Onda 3.",
        )

        for family_key in WAVE_3_ACTIVE_FAMILIES:
        ...
        commit_ou_rollback_integridade(
            db,
            logger_operacao=logging.getLogger("tariel.wave3.homologation"),
            mensagem_erro="Falha ao emitir demos homologadas da Onda 3.",
        )
        ...
        json.dumps(
            {
                ...
                "familias_ativas": len(WAVE_3_ACTIVE_FAMILIES),
