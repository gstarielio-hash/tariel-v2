...
_PREVIEW = "Attachment smoke draft"
_THREAD_MESSAGE = "Caso editavel preparado para smoke de anexos no emulador."
...
def _resolve_inspector(banco, email: str) -> Usuario:
    ...
    if usuario is None:
        raise RuntimeError(f"Inspetor do smoke de anexos nao encontrado: {email}")
...
def _ensure_seed_laudo(banco, *, inspector: Usuario) -> Laudo:
    ...
    if laudo is None:
        laudo = Laudo(
            ...
            setor_industrial="Attachment Smoke",
            tipo_template="padrao",
            status_revisao=StatusRevisao.RASCUNHO.value,
            codigo_hash=uuid.uuid4().hex,
            primeira_mensagem=_PREVIEW,
            parecer_ia="Laudo seed local para smoke de anexos no emulador.",
    ...
    laudo.usuario_id = inspector.id
    laudo.empresa_id = inspector.empresa_id
    laudo.setor_industrial = "Attachment Smoke"
    laudo.tipo_template = "padrao"
    laudo.status_revisao = StatusRevisao.RASCUNHO.value
    laudo.primeira_mensagem = _PREVIEW
    laudo.parecer_ia = "Laudo seed local para smoke de anexos no emulador."
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Garante um laudo editavel para smoke Maestro de anexos no emulador."
    )
