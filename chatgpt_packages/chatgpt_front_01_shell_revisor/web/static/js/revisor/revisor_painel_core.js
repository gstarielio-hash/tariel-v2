    ...
    const atualizarIndicadoresListaLaudo = (laudoId, {
        ...
        document.querySelectorAll(`.js-item-laudo[data-id="${CSS.escape(String(alvo))}"]`).forEach((itemEl) => {
            ...
            if (collaborationSummary) {
                proximoWhispers = collaboration.unreadWhisperCount;
                proximoPendencias = collaboration.openPendencyCount;
            ...
            if ((proximoWhispers === null || proximoWhispers === undefined) && delta) {
                const atualWhispers = Math.max(0, Number(itemEl.dataset.whispersNaoLidos || 0) || 0);
                proximoWhispers = atualWhispers + delta.unreadWhisperDelta;
            }

            if (proximoWhispers !== null && proximoWhispers !== undefined) {
                const totalWhispers = Math.max(0, Number(proximoWhispers || 0) || 0);
            ...
            if (proximoPendencias !== null && proximoPendencias !== undefined) {
                const totalPendencias = Math.max(0, Number(proximoPendencias || 0) || 0);
            ...
            const collaborationAtualizada = normalizarCollaborationSummary(
                collaborationSummary || collaborationFromDataset || {},
                {
                    whispersNaoLidos: itemEl.dataset.whispersNaoLidos || 0,
                    pendenciasAbertas: itemEl.dataset.pendenciasAbertas || 0
                }
            ...
            itemEl.dataset.collaborationSummary = JSON.stringify({
                open_pendency_count: collaborationAtualizada.openPendencyCount,
                resolved_pendency_count: collaborationAtualizada.resolvedPendencyCount,
                recent_whisper_count: collaborationAtualizada.recentWhisperCount,
                unread_whisper_count: collaborationAtualizada.unreadWhisperCount,
                recent_review_count: collaborationAtualizada.recentReviewCount,
                has_open_pendencies: collaborationAtualizada.hasOpenPendencies,
                has_recent_whispers: collaborationAtualizada.hasRecentWhispers,
                requires_reviewer_attention: collaborationAtualizada.requiresReviewerAttention
            });
